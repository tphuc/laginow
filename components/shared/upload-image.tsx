...
const ImageUploader: React.FC<ImageUploaderProps> = ({ className, images, onChange, placeholder }) => {
  ...
  return (
    <div className={cn("grid grid-cols-2 gap-1.5 lg:grid-cols-4 w-full", className)}>
      <input onChange={onUserSelect} id={`upload_${uploadId}`} type="file" className="hidden" />
      <div className={`relative flex items-center border-1.5 border-gray-200 shadow shadow-medium rounded-md bg-zinc-50 justify-center aspect-square rounded-md`}>
        <label htmlFor={`upload_${uploadId}`} className="flex w-[90%] h-[90%] flex-col items-center justify-center gap-1 cursor-pointer text-gray-400">
          <ImagePlus />
      ...
      {loading && <div className={`relative aspect-square overflow-hidden rounded-lg border border-gray-200 shadow-sm p-4 shadow-xl shadow-black/5 before:absolute before:inset-0 before:rounded-lg before:-translate-x-full  before:animate-[shimmer_1.4s_infinite]  before:bg-gradient-to-r before:from-white before:via-gray-200 before:to-transparent`} />}

      {list?.map((item, id) => <div key={id} className={`relative aspect-square box-border rounded-md shadow-sm shadow-sm`}>
        <Image src={item?.url} alt="" fill className="appearance-none w-full object-cover bg-center aspect-square rounded-md" placeholder="empty" loading="lazy" />
        <div onClick={() => handleRemove(item.fileId)} style={{ zIndex: 10 }} className='absolute rounded-xl flex bg-gray-50 items-center justify-center  bg-white shadow-sm border border-gray-200 p-0.5 top-[-8px] right-[-8px]'>
